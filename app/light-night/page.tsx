...
export default function LightNightPage() {
  ...
  const upcoming: EncounterWithStart[] = sorted.filter(
    (e) => e._startTime !== null && (e._startTime as number) >= now
  );

  const past: EncounterWithStart[] = sorted.filter(
    (e) => e._startTime === null || (e._startTime as number) < now
  );
  ...
  // --- PAST SLIDER STATE & DERIVED VALUES ---

  const [pastIndex, setPastIndex] = React.useState<number>(0);

  const currentPast = React.useMemo<EncounterWithStart | null>(() => {
    if (!past || past.length === 0) return null;
    const clampedIndex = Math.min(Math.max(pastIndex, 0), past.length - 1);
    return past[clampedIndex];
  }, [past, pastIndex]);

  const currentMonthYear = React.useMemo<string>(() => {
    if (!currentPast) return "";

    let d: Date | null = null;

    if (currentPast.startDate) {
      d = new Date(currentPast.startDate);
    } else if (typeof currentPast._startTime === "number") {
      d = new Date(currentPast._startTime);
    }
          ...
          <div className="max-w-xl space-y-4">
            ...
            <Reveal delayMs={80}>
              ...
              <p className="text-xs sm:text-sm text-slate-400">
                Light Night is part of Alabaster Ministries&apos;s rhythm — a
                recurring worship and prayer night where the whole family and
                friends can gather around Jesus.
              </p>
            </Reveal>
          </div>
                    ...
                    <div className="absolute bottom-4 left-4 right-4 space-y-1 text-sm">
                      ...
                      <p className="text-[11px] text-slate-300">
                        {nextEncounter.church
                          ? `${nextEncounter.church} • ${nextEncounter.city ?? nextEncounter.location
                          }`
            ...
            <div className="space-y-4">
              ...
              {otherUpcoming.length === 0 ? (
                <p className="text-xs text-slate-400">
                  Once we schedule more Light Night worship nights, you&apos;ll
                  see them here.
                </p>
              ) : (
